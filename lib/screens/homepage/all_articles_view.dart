...
import 'package:youthspot/global_widgets/empty_state_widget.dart';
...
class _AllArticlesViewState extends State<AllArticlesView> {
  ...
  Widget build(BuildContext context) {
    ...
    return PrimaryScaffold(
      child: articlesProvider.hasError || articlesProvider.allArticles.isEmpty
          ? EmptyStateWidget(
              message: articlesProvider.hasError
                  ? 'Content will load when connected'
                  : 'No articles available',
              icon: Icons.article_outlined,
              onRetry: articlesProvider.hasError ? articlesProvider.retry : null,
              showRetryButton: articlesProvider.hasError,
            )
          : PrimaryPadding(
